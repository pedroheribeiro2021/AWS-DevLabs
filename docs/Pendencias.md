...
- **Gamification, phase 2 — badges/achievements**: Session 14 shipped XP/levels/streaks (ADR 0005). Pedro also wants badges (milestones like "first lab completed", "domain mastered", "7-day streak") and a visual skill-tree-style track replacing the flat topic lists — both deferred by explicit agreement to keep the first slice small. Badges likely need an XP-event ledger (not required for XP/streaks alone) to react to specific events rather than just totals.
- **Flashcards excluded from XP-awarding**: the spaced-repetition review flow is repeatable by design, which doesn't fit the "award once on first completion" gating used for lessons/labs/questions/simulations. Needs its own design pass (award per state transition? only on reaching MASTERED? every review, capped?) — bundle with the badges work.
