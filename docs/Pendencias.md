...
- **Migrate `apps/api` off Vercel to Render**: production API hangs indefinitely on every request (TLS to Vercel's edge completes instantly, but the Lambda function itself never returns — zero application logs, confirmed via two independent network paths). Four targeted, individually-verified-locally fixes in a row (Node 22.x pin, `@prisma/adapter-pg` version-mismatch fix, switch to `@prisma/adapter-neon`'s WebSocket driver, env-conditional adapter selection) produced the exact same symptom in production every time — strong evidence the problem is in Vercel's Lambda networking for this project, not in the application code paths changed across those four fixes. See ADR 0004's update for the full trail. Decision: move `apps/api` to Render tomorrow; `apps/web` stays on Vercel (frontend works fine there).
