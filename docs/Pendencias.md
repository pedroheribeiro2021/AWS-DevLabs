...
- **Confirm the keep-alive schedule actually fires**: the manual `workflow_dispatch` run of `keep-api-warm.yml` succeeded (2026-09-26 12:23 Brasília), but ~20 min after merge no scheduled run had happened yet — GitHub can take a while to pick up a new schedule and may delay runs. Check `gh run list --workflow keep-api-warm.yml` for `schedule` events; if they never appear or are too sparse, consider an external pinger.
