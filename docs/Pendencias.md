...
- **Content needed for 5 of the 12 empty topics, plus reinforcing the 3 topics written before the exam-readiness bar (Session 21)**: tracked with real volume estimates (lessons/labs/questions/flashcards per topic, updated as each one ships) in `docs/Conteudo-DVA-C02.md`, not just this line. The Learning track and Simulations already handle empty topics gracefully in the meantime.
...
- **Confirm the keep-alive schedule actually fires**: the manual `workflow_dispatch` run of `keep-api-warm.yml` succeeded (2026-09-26 12:23 Brasília), but no scheduled run had happened by 13:08 Brasília, ~47 min after the workflow was activated (12:21) — GitHub can take a while to pick up a new schedule and may delay or drop runs. Check `gh run list --workflow keep-api-warm.yml` for `schedule` events; if they never appear or are too sparse, consider an external pinger.
